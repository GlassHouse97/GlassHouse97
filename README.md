## 🙋‍♂️ Hi there !! I'm Chris Delgado


Hello, I’m **Chris Delgado** — a **Data Analyst** with experience turning raw data into clear, actionable insights.  
I specialize in SQL-driven analysis, data modeling, and building dashboards that support business decision-making.

**What I do best:**
- Query, clean, and analyze data using **SQL**
- Perform ad-hoc analysis and reporting to answer business questions
- Design and optimize datasets for analytics use cases
- Build interactive dashboards and reports for stakeholders
- Work with spreadsheets for exploratory analysis and validation

---

### 📚 Projects

Welcome to my portfolio, where I showcase selected projects demonstrating my analytical and technical skills:

Each project includes:
- Business context & problem statement  
- SQL queries or analysis approach  
- Key insights and outcomes  
- Visualizations or dashboards (where applicable)

---

### 🛠️ Tools & Technologies

- **Programming Languages:** SQL, Python, R, M/PowerQuery
- **Tools/Technologies:** Google BigQuery, Looker Studio, Power BI  

---

### 👋 Connect With Me

- 💼 **LinkedIn:**  [LinkedIn](https://www.linkedin.com/in/christopherdelgadospage/)  
- 📧 **Email:** chrisdelgado.mail@gmail.com



---

### 📊 Currently Working On
- Improving query performance on large datasets  
- Expanding my portfolio with end-to-end analytics case studies  ## Introducing Myself 👋

<!--
**GlassHouse97/GlassHouse97** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
